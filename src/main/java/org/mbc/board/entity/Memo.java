...
public class Memo {
    ...
    // GenerationType.IDENTITY =>pk를 자동으로 생성하고자 함(키생성 전략)
    // 만일 연결되는 데이터베이스가 오라클이면 번호를 위한 별도의 테이블을 생성(시퀀스객체)
    // Mysql 이나 MariaDB면 auto increment를 기본으로 사용해서 새로운 레코드가 기록될 때 다른 번로를 줌
